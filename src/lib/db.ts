  ...
  if (!dbPromise) {
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        // Création des object stores et indexes
        ...
        if (!db.objectStoreNames.contains('classroomStudents')) {
          ...
          csStore.createIndex('studentId', 'studentId', { unique: true });
...
export async function createClassroom(classroomData: ClassroomFormData): Promise<Classroom> {
  ...
  const classroom: Classroom = {
    ...
    section: classroomData.section as Classroom['section'],
...
export async function getClassroomsForTeacher(teacherId: string): Promise<Classroom[]> {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      // Classes where the teacher is assigned as teacher
      const tx1 = db.transaction(CLASSROOMS_STORE, 'readonly');
      const store1 = tx1.objectStore(CLASSROOMS_STORE);
      const index1 = store1.index('teacherId');
      const request1 = index1.getAll(teacherId);

      // Classes where the teacher teaches courses
      const tx2 = db.transaction('courseAssignments', 'readonly');
      const store2 = tx2.objectStore('courseAssignments');
      const index2 = store2.index('teacherId');
      const request2 = index2.getAll(teacherId);

      const [classes1, assignments] = await Promise.all([
        new Promise<Classroom[]>((res) => {
          request1.onsuccess = () => res(request1.result || []);
          request1.onerror = () => res([]);
        }),
        new Promise<CourseAssignment[]>((res) => {
          request2.onsuccess = () => res(request2.result || []);
          request2.onerror = () => res([]);
        })
      ]);
      ...
      const classrooms = await Promise.all(
        classroomIds.map(id => getClassroomById(id))
      );

      resolve(classrooms.filter(c => c !== null) as Classroom[]);
    } catch (error) {
      reject(error);
    }
  });
...
// Fonctions pour les étudiants dans les classes
    ...
    try {
      ...
      request.onsuccess = async () => {
        ...
        const students = await Promise.all(relations.map(relation => 
          new Promise<Student>((res, rej) => {
            const studentRequest = studentStore.get(relation.studentId);
            studentRequest.onsuccess = () => res(studentRequest.result);
            studentRequest.onerror = () => rej(studentRequest.error);
          })
        ));
...
export async function recordAbsences(
  ...
  await Promise.all(absentStudentIds.map(studentId => 
  ...
  // Enregistrer la session d'absences
  return new Promise((resolve, reject) => {
...
export async function getAbsenceStatistics(classroom_id: string): Promise<{
  ...
  // Obtenir toutes les absences pour la classe
  const absences: Absence[] = await new Promise((resolve, reject) => {
    const tx = db.transaction('absences', 'readonly');
    const store = tx.objectStore('absences');
    const index = store.index('by_classroom');
    const request = index.getAll(classroom_id);
  ...
  // Obtenir les étudiants de la classe
