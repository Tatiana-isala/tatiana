...
//     if (messageType === 'MESSAGE' && !recipientId) {
...
//             <button
//               className={`py-2 px-4 font-medium ${activeTab === 'inbox' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
...
//             {canSendMessages() && (
//               <button
//                 className={`py-2 px-4 font-medium ${activeTab === 'compose' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}
//                 onClick={() => {
//                   setActiveTab('compose');
//                   setReplyingTo(null);
//                 }}
...
//                     <div 
//                       className={`p-4 ${thread.isRead ? 'bg-white' : 'bg-blue-50'}`}
//                       onClick={() => !thread.isRead && markAsRead(thread.id)}
//                     >
...
//                           <div key={reply.id} className="p-4 border-b last:border-b-0">
//                             <div className="flex justify-between items-start">
//                               <div>
...
//                     En réponse à: <span className="font-medium">{replyingTo.title}</span>
//                   </p>
//                 </div>
//               )}
...
//                   disabled={isLoading || !newMessage || !newTitle || (messageType === 'MESSAGE' && !recipientId)}
//                 >
  ...
  const loadParentsByClassroom = async (classroomId: string) => {
    ...
    try {
      ...
      const classroomParents = allParents.filter(parent => 
        students.some(student => student.tuteurId === parent.id)
      );
